...
function bds_register_script ()
{
	...
	$api = 'https://maps.googleapis.com/maps/api/js?libraries=places&key='. $key. '&callback=Function.prototype';
//
...
function bds_save_location ($data)
{
	...
	$table_name = $wpdb->prefix. 'bds_locations';
	$lat = $_POST['Lat_field_'. $field_id];
	$lng = $_POST['Lng_field_'. $field_id];

	if ($lat === '' && $lng === '')
	{
		$query = "SELECT * FROM $table_name WHERE user_id = %d AND field_id = %d;";
		$query = $wpdb->prepare ($query, $user_id, $field_id);
		$row = $wpdb->get_row ($query);
		if (!empty ($row->location) && $row->location == $value)  return false;
		
		if (is_admin ())
		{
			$data->value = !empty ($row->location)? $row->location: '';
			return false;
		}

		$redirect_url = trailingslashit (bp_displayed_user_domain(). bp_get_profile_slug(). '/edit/group/'. bp_action_variable(1));
		bp_core_add_message (sprintf (__('%s: Please select a location from the dropdown list!', 'bp-distance-search'), "<strong>$field->name</strong>"), 'error');
	...
	$query = $wpdb->prepare ($query, $user_id, $field_id, $value, $lat, $lng, $value, $lat, $lng);
	$wpdb->query ($query);
}

add_action ('bp_signup_validate', 'bds_signup_validate');
function bds_signup_validate ()
{
	global $bp;

	$field_ids = empty ($_POST['signup_profile_field_ids'])? array (): explode (',', $_POST['signup_profile_field_ids']);
	foreach ($field_ids as $field_id)
	{
		$field = new BP_XProfile_Field ($field_id);
		if ($field->type != 'bds_location')  continue;

		$lat = $_POST['Lat_field_'. $field_id];
		$lng = $_POST['Lng_field_'. $field_id];

		if ($lat === '' && $lng === '')
			$bp->signup->errors['field_'. $field_id] = __('Please select a location from the dropdown list!', 'bp-distance-search');
	}
}
